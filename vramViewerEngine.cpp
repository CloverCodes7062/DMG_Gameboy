...
vramViewerEngine::vramViewerEngine(int width, int height) : screenWidth(width), screenHeight(height), window(nullptr), renderer(nullptr), texture(nullptr) 
{
}

vramViewerEngine::~vramViewerEngine()
{
...
void vramViewerEngine::createWindow()
{
    if (!window)
    {
...
void vramViewerEngine::destroyWindow()
{
    if (window) 
    {
        SDL_DestroyRenderer(renderer);
...
void vramViewerEngine::setBuffer(uint8_t vramDataArray[6144])
{
    int bufferIndex = 0;
...
void vramViewerEngine::renderDebug() {
